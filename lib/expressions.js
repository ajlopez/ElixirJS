...
function DefExpression(name, argnames, exprs) {
    this.evaluate = function (ctx) {
        var fn = fns.fn(name, argnames, exprs, ctx);
        ctx.set(name, fn);
        return fn;
    }
}

function IfExpression(condexpr, thenexpr, elseexpr) {
    this.evaluate = function (ctx) {
        if (condexpr.evaluate(ctx))
...
function ConsExpression(headexpr, tailexpr) {
    this.evaluate = function (ctx) {
        var head = headexpr.evaluate(ctx);
        var tail = tailexpr.evaluate(ctx);
        
        return lists.list(head, tail);
    };
}
...
function createCons(headexpr, tailexpr) {
    return new ConsExpression(headexpr, tailexpr);
}
...
function createDef(name, argnames, exprs) {
    return new DefExpression(name, argnames, exprs);
}


function createIf(condexpr, thenexpr, elseexpr) {
    return new IfExpression(condexpr, thenexpr, elseexpr);
}
...
module.exports = {
    ...
    match: createMatch,
    append: createAppend,
    subtractList: createSubtractLists,
    call: createCall,
    dot: createDot,
    def: createDef,
    if: createIf
