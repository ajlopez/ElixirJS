...
var TokenType = { Name: 1, Integer: 2, Real: 3, String: 4, Atom: 5, Operator: 6, Delimiter: 7  };

var operators = [ "=>", "=", "+", "-", "*", "/", "==", "!=", "<", ">", "<=", ">=", "++", "--", "<>", "!", "&&", "||", "|", "not", "and", "or", "xor", "div", "rem", "in", "when", "|>", "=~", "..", "->" ];
var delimiters = [ "{", "}", "[", "]", ",", "(", ")", ">>", "<<", ".", ";" ];
...
function Lexer(text) {
    ...
    function nextInteger(value) {
        for (var ch = text[position++]; isDigit(ch) || ch === '_'; ch = text[position++])
            if (ch !== '_')
                value += ch;
    ...
    function nextReal(value) {
        ...
        var token = { value: value, type: TokenType.Real };
