...
function Parser(text) {
    ...
    function parseBinaryExpression() {
        ...
        if (tryParseToken("=", TokenType.Operator))
            return expressions.match(expr, self.parseExpression());

        if (tryParseToken("+", TokenType.Operator))
            return expressions.add(expr, self.parseExpression());
            
        if (tryParseToken("-", TokenType.Operator))
            return expressions.subtract(expr, self.parseExpression());
            
        if (tryParseToken("*", TokenType.Operator))
            return expressions.multiply(expr, self.parseExpression());
            
        if (tryParseToken("/", TokenType.Operator))
            return expressions.divide(expr, self.parseExpression());
            
        if (tryParseToken("++", TokenType.Operator))
            return expressions.append(expr, self.parseExpression());
            
        if (tryParseToken("--", TokenType.Operator))
            return expressions.subtractl(expr, self.parseExpression());
            
        if (tryParseToken("<>", TokenType.Operator))
            return expressions.concat(expr, self.parseExpression());
