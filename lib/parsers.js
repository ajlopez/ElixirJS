    ...
    function parseBinaryExpression(level) {
        ...
        for (var token = lexer.nextToken(); token && token.type == TokenType.Operator && binops[level].indexOf(token.value) >= 0; token = lexer.nextToken()) {
            if (token.value === "=")
                ...
                expr = expressions.equal(expr, parseBinaryExpression(level + 1));
            else if (token.value === "!=")
                expr = expressions.notEqual(expr, parseBinaryExpression(level + 1));
            else if (token.value === "<")
                expr = expressions.less(expr, parseBinaryExpression(level + 1));
            else if (token.value === "<=")
                expr = expressions.lessEqual(expr, parseBinaryExpression(level + 1));
            else if (token.value === ">")
                expr = expressions.greater(expr, parseBinaryExpression(level + 1));
            else if (token.value === ">=")
                expr = expressions.greaterEqual(expr, parseBinaryExpression(level + 1));
        }
